...
def decode(instr):
    if not all(c in "01" for c in instr):
        raise Exception(f"Invalid binary instruction: {repr(instr)}")
    if len(instr) != 32:
...
def load_word(addr):
    ...
    if DATA_BASE <= addr < DATA_BASE + 128:
        return data_mem[(addr - DATA_BASE) // 4]
    elif STACK_BASE <= addr < STACK_BASE + 128:
...
def execute(d, reg, pc, old_pc):
    ...
    if d["type"] == "R":
        ...
        if inst == "add":
            ...
            reg[rd] = (to_signed(reg[rs1]) >> (reg[rs2] & 0x1F)) & 0xFFFFFFFF
        ...
        if d["inst"] == "lui":
            ...
            reg[d["rd"]] = (old_pc + (d["imm"] << 12)) & 0xFFFFFFFF
        pc = (pc + 4) & 0xFFFFFFFF
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(instructions) == 0:
            print(f" ERROR: The input file '{input_file}' is empty or has no valid 32-bit instructions!")
            sys.exit(1)
        ...
        print(f"Success! Trace saved to {output_file}")
